...
public class CollectionUtil {
    ...
    public static <T> Collection<T> subCollection(final Collection<T> collection, final int offset, final int limit) {
        ...
        final Collection<T> rt = new ArrayList<>();
        int index = 0;
    ...
    public static boolean contains(final Iterable<?> iterable, final Object element) {
        for (final Object o : iterable) {
            if (Kits.OBJ.deepEquals(o, element)) {
                return true;
            }
        }
        return false;
    ...
    @SuppressWarnings("unchecked")
    private static final ElementTransformer<Object, Object> NO_TRANS = (index, from) -> from;
    ...
    public static <T> ElementTransformer<T, T> noTrans() {
        return (ElementTransformer<T, T>) NO_TRANS;
    }
     ...
     * @param <E>
     * @param source 源集合.
     * @param filter 过滤器.只有符合过滤器的元素才会被处理.若过滤器为null,则处理所有元素.
     * @param excutor 元素操作者.详参此接口本身.
     * @param offset 开始处理的元素的下标.(即"本页"之前的元素们)
     * @param size 处理元素的个数.
     */
    public static <E> void handleCollectionElement(final Collection<? extends E> source,//
                                                   final ElementFilter<? super E> filter,
                                                   final ElementHandler<E> excutor, int offset, int size) {
        if (source == null) {
            return;
        }
        if (offset < 0) {
